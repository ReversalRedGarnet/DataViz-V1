...
// now derives its list from this one. `source.label` stays English-only: it
// is the dataset's own name at the Pacific Data Hub, a proper-noun-like
// citation rather than site copy, the same way a bibliography entry doesn't
// get translated.
//
...
//
// TRANSLATION-ERA CHANGE: `label` and `caveat` are now { en, fr } objects
// rather than plain strings -- call metricLabel(m, language) / metricCaveat(m,
// language) below rather than reading .label/.caveat directly. `format` is
// deliberately NOT translated in this pass: its output is axis ticks and
// short in-chart figures (units like "kg/ha", "GWh", "°C" read the same in
// French; "people"/"visitors"/"stations" don't yet). That's a known, flagged
// gap rather than an oversight -- see the French-translation build notes.
