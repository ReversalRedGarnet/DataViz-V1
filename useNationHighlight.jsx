...
const HighlightContext = createContext({ highlight: null, setHighlight: () => {} })
...
export function NationHighlightProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const wanted = highlight ? `nation-${slug(highlight)}` : null
  ...
  // Cleared on any pointer leaving the subtree, so a highlight can't survive
  // the cursor moving to another part of the page.
